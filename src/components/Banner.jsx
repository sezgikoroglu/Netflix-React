...
import { getTopRatedTv } from "../services/tv";
...
const Banner = ({type}) => {
  ...
  useEffect(()=>{
    type==="tv"? dispatch(getTopRatedTv()) : dispatch(getTopRatedMovies())
  },[])

  useEffect(() => {
    if (topRatedList) {
      setMovie(topRatedList[Math.floor(Math.random() * topRatedList.length)])
    }
  }, [topRatedList]);
console.log(movie)
  return (
